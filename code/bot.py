...
async def start(update, context):
    await update.message.reply_text("🏏 Welcome! Use /matches for match info and /score for live scores.")

async def matches(update, context):
    await update.message.reply_text("📡 Fetching match details...")
    info = get_match_details()
    await send_long_message(info, update, parse_mode='Markdown')
...
async def send_long_message(text, update, parse_mode='Markdown'):
    for i in range(0, len(text), 4096):
        await update.message.reply_text(text[i:i+4096], parse_mode=parse_mode)

def main():
    # New v20+ initialization
    application = Application.builder().token(TELEGRAM_BOT_TOKEN).build()
    
    # Add handlers (unchanged syntax)
